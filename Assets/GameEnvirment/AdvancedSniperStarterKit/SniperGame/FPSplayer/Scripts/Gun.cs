		...
		case 0:
			...
			if (AmmoIn <= 0) 
			{
				...
				else 
				{
					...
					//animation.Play(ShootPose);
					//animation.clip = animation [ShootPose].clip;
					//gunState = 2;
					//audiosource.PlayOneShot (SoundBoltStart);
					//animation.Play("Fire");
					gunState = 3;	
				...
				if (AmmoIn > 0) {
					...
					for (int i=0; i<BulletNum; i++) {
						if (Bullets) {
							...
							if(BulletsCounter.gun==true)
								BulletsCounter.bullet_G=BulletsCounter.bullet_G-1;
							if(BulletsCounter.pistal==true)
								BulletsCounter.bullet_P=BulletsCounter.bullet_P-1;
